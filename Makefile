...
test: manifests generate fmt vet setup-envtest ## Run tests.
	...
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" ginkgo -v --trace --cover --fail-on-pending ./internal/...
...
KIND_CLUSTER ?= orangectl-test-e2e
...
setup-test-e2e: ## Set up a Kind cluster for e2e tests if it does not exist
	...
	if echo "$$CLUSTERS" | grep -q "$(KIND_CLUSTER)"; then \
		...
		$(KIND) load docker-image np137270/orangectl:latest --name  $(KIND_CLUSTER); \
		$(KIND) load docker-image np137270/orange:latest --name  $(KIND_CLUSTER); \
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name orangectl-builder
	$(CONTAINER_TOOL) buildx use orangectl-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm orangectl-builder
	rm Dockerfile.cross
